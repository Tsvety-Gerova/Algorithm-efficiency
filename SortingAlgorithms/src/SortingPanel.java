...
class SortingPanel extends JPanel {
	...
	int currentI = -1;
	int currentJ = -1;
	boolean changeOccurred = false;// if for the current i there have been any changes
	...
	public void nextStepInSorting() {
		...
		if (stage == incrementJStage) {
			...
			if (numberJ > numberJPlus1) {
				changeOccurred = true; // something has been changed
	...
	private void nextStage() {
		if (stage == incrementJStage) {
			...
			if (currentJ < numbers.size() - 2 - currentI) { 
				...
				if (currentI < numbers.size() - 2 && changeOccurred) {
